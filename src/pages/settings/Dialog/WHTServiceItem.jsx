...
import {
  getWhtServiceDetail,
  createWhtServiceDetail,
  updateWhtServiceDetail,
  delWhtServiceDetail,
} from "services/accountPayable";
import Model from "models/apWhtService";
...
export default function DialogItem(props) {
  ...
  const fetchDetailById = useCallback(async () => {
    if (id && id !== 0) {
      const response = await getWhtServiceDetail(id);
      if (response) {
        setData(response);
        reset(response);
      }
  ...
  const onSubmit = (values) => {
    values.WhtCode = values.WhtCode.replace(/ /g, "");
  ...
  const Save = async (values) => {
    if (mode === "edit") {
      values.Id = id;
      //Update
      const { Code, UserMessage } = await updateWhtServiceDetail(values);
      if (Code === 0) {
      ...
      console.log(values);
      const { Code, InternalMessage, UserMessage } = await createWhtServiceDetail(values);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
  ...
  const DelOrVoid = async (id) => {
    ...
    if (dialog) {
      const { Code, UserMessage } = await delWhtServiceDetail(id);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
        handleClose(id);
      }
