...
import { getApWhtTypeSearchList } from "services/accountPayable";
import MUIDataTable from "mui-datatables";
import ActionMenu from "components/ActionMenu";
import DialogItem from "./WHTTypeItem";
...
const DialogWHTType = (props) => {
  ...
  const fetchSearchList = async (uriQueryString) => {
    const { Data, Paging } = await getApWhtTypeSearchList(uriQueryString);
    if (Data) {
      setData(Data);
      setPaging(Paging);
    ...
    searchProps: {
      onKeyUp: (e) => {
        if (e.keyCode === 13) {
          ...
          uriQueryString.WhereLikeFields = ["WhtTypeCode", "WhtTypeDesc"];
          fetchSearchList(uriQueryString);
        }
      },
    },
