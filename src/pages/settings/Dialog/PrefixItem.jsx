...
import {
  getGlPrefixDetail,
  createGlPrefixDetail,
  updateGlPrefixDetail,
  delGlPrefixDetail,
} from "services/generalLedger";
import Model from "models/glPrefix";
...
export default function DialogItem(props) {
  ...
  const fetchDetailById = useCallback(async () => {
    if (id && id !== 0) {
      const response = await getGlPrefixDetail(id);
      if (response) {
        setData(response);
        reset(response);
      }
  ...
  const onSubmit = (values) => {
    values.PrefixName = values.PrefixName.replace(/ /g, "");
  ...
  const Save = async (values) => {
    if (mode === "edit") {
      values.Id = id;
      //Update
      const { Code, UserMessage } = await updateGlPrefixDetail(values);
      if (Code === 0) {
      ...
      console.log(values);
      const { Code, InternalMessage, UserMessage } = await createGlPrefixDetail(values);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
  ...
  const DelOrVoid = async (id) => {
    ...
    if (dialog) {
      const { Code, UserMessage } = await delGlPrefixDetail(id);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
        handleClose(id);
      }
