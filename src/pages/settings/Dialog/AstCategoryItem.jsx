...
import {
  getAstCategoryDetail,
  createAstCategoryDetail,
  updateAstCategoryDetail,
  delAstCategoryDetail,
} from "services/asset";
import Model from "models/astCategory";
...
export default function DialogItem(props) {
  ...
  const fetchDetailById = useCallback(async () => {
    if (id && id !== 0) {
      const response = await getAstCategoryDetail(id);
      if (response) {
        setData(response);
        reset(response);
      }
  ...
  const onSubmit = (values) => {
    values.CateCode = values.CateCode.replace(/ /g, "");
  ...
  const Save = async (values) => {
    if (mode === "edit") {
      values.Id = id;
      //Update
      const { Code, UserMessage } = await updateAstCategoryDetail(values);
      if (Code === 0) {
      ...
      console.log(values);
      const { Code, InternalMessage, UserMessage } = await createAstCategoryDetail(values);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
  ...
  const DelOrVoid = async (id) => {
    ...
    if (dialog) {
      const { Code, UserMessage } = await delAstCategoryDetail(id);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
        handleClose(id);
      }
