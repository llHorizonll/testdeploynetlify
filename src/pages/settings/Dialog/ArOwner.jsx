...
import { getArOwnerSearchList } from "services/accountReceivable";
import MUIDataTable from "mui-datatables";
import ActionMenu from "components/ActionMenu";
import DialogItem from "./ArOwnerItem";
...
const DialogArOwner = (props) => {
  ...
  const fetchSearchList = async (uriQueryString) => {
    const { Data, Paging } = await getArOwnerSearchList(uriQueryString);
    if (Data) {
      setData(Data);
      setPaging(Paging);
    ...
    searchProps: {
      onKeyUp: (e) => {
        if (e.keyCode === 13) {
          ...
          uriQueryString.WhereLikeFields = ["ArOwnerCode", "Description"];
          fetchSearchList(uriQueryString);
        }
      },
    },
