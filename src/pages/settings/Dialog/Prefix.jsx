...
import { getGlPrefixSearchList } from "services/generalLedger";
import MUIDataTable from "mui-datatables";
import ActionMenu from "components/ActionMenu";
import DialogItem from "./PrefixItem";
...
const DialogPrefix = (props) => {
  ...
  const fetchSearchList = async (uriQueryString) => {
    const { Data, Paging } = await getGlPrefixSearchList(uriQueryString);
    if (Data) {
      setData(Data);
      setPaging(Paging);
    ...
    searchProps: {
      onKeyUp: (e) => {
        if (e.keyCode === 13) {
          ...
          uriQueryString.WhereLikeFields = ["PrefixName", "PrefixDesc"];
          fetchSearchList(uriQueryString);
        }
      },
    },
