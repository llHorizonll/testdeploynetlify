...
import { getArProjectSearchList } from "services/accountReceivable";
import MUIDataTable from "mui-datatables";
import ActionMenu from "components/ActionMenu";
import DialogItem from "./ArProjectItem";
...
const DialogArProject = (props) => {
  ...
  const fetchSearchList = async (uriQueryString) => {
    const { Data, Paging } = await getArProjectSearchList(uriQueryString);
    if (Data) {
      setData(Data);
      setPaging(Paging);
    ...
    searchProps: {
      onKeyUp: (e) => {
        if (e.keyCode === 13) {
          ...
          uriQueryString.WhereLikeFields = ["ArPrjCode", "Description"];
          fetchSearchList(uriQueryString);
        }
      },
    },
...
export default DialogArProject;
