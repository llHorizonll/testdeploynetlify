...
import {
  getAstLocationDetail,
  createAstLocationDetail,
  updateAstLocationDetail,
  delAstLocationDetail,
} from "services/asset";
import Model from "models/astLocation";
...
export default function DialogItem(props) {
  ...
  const fetchDetailById = useCallback(async () => {
    if (id && id !== 0) {
      const response = await getAstLocationDetail(id);
      if (response) {
        setData(response);
        reset(response);
      }
  ...
  const onSubmit = (values) => {
    values.LocCode = values.LocCode.replace(/ /g, "");
  ...
  const Save = async (values) => {
    if (mode === "edit") {
      values.Id = id;
      //Update
      const { Code, UserMessage } = await updateAstLocationDetail(values);
      if (Code === 0) {
      ...
      console.log(values);
      const { Code, InternalMessage, UserMessage } = await createAstLocationDetail(values);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
  ...
  const DelOrVoid = async (id) => {
    ...
    if (dialog) {
      const { Code, UserMessage } = await delAstLocationDetail(id);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
        handleClose(id);
      }
