...
import Model from "models/arTitle";
...
export default function DialogItem(props) {
  ...
  const fetchDetailById = useCallback(async () => {
    if (id && id !== 0) {
      const response = await getArTitleDetail(id);
      if (response) {
        setData(response);
        reset(response);
      }
  ...
  const onSubmit = (values) => {
    values.ArTitleCode = values.ArTitleCode.replace(/ /g, "");
  ...
  const Save = async (values) => {
    if (mode === "edit") {
      values.Id = id;
      //Update
      const { Code, UserMessage } = await updateArTitleDetail(values);
      if (Code === 0) {
      ...
      console.log(values);
      const { Code, InternalMessage, UserMessage } = await createArTitleDetail(values);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
  ...
  const DelOrVoid = async (id) => {
    ...
    if (dialog) {
      const { Code, UserMessage } = await delArTitleDetail(id);
      if (Code === 0) {
        SnackbarUtils.success(UserMessage);
        handleClose(id);
      }
