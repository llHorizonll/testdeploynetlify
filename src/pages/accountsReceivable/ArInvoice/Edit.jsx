...
const Edit = (props) => {
  ...
  const { basePath, id, formFields, formFieldsDetail, arProfileList, copyMode, translate } = props;
  ...
  const UpdateForm = (e) => {
    ...
    if (arItem && ToNumber(arItem.CreditTerm) !== 0 && e.target.name !== "InvhDate") {
      let invDate = new Date(values.InvhDate);
      let newDueDate = addDays(invDate, parseInt(arItem.CreditTerm));
      setValue("InvhDueDate", newDueDate);
    }

    if (e.target.name === "InvhDate") {
      let newDate = new Date(e.target.value);
      let creditTerm = parseInt(arItem.CreditTerm);
  ...
  const Save = async (values) => {
    ...
    //Update
    if (!copyMode) {
      const { Code, InternalMessage } = await updateArInvoiceDetail(values);
      if (Code === 0) {
    ...
    >
      ...
      <form onSubmit={handleSubmit(onSubmit)} onKeyDown={disableFormEnter}>
        <Paper style={{ padding: 16 }}>
          <BoxHeader header={`Invoice`} status={data.InvhStatus} />
