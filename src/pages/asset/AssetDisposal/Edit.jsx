...
import { Paper, Grid, Typography, TextField, Divider } from "@material-ui/core";
...
const Edit = (props) => {
  ...
  const { basePath, id, formFieldsSale, formFieldsDisposal, formFieldsAccount } = props;
  ...
  const fetchAssetDisItem = useCallback(async () => {
    setLoading(true);
    ...
    if (response) {
      ...
      let paramAstDisposalQuery = {
        DisposalDate: new Date(response.DisposalDate),
        Id: response.Id,
        No: response.No,
        Type: response.Type,
        Qty: response.Qty,
  ...
  const Save = async (values) => {
    ...
    const { Code, InternalMessage } = await updateAssetDisDetail(values);
    if (Code === 0) {
    ...
    if (regData) {
      return (
        <Grid container justifyContent="flex-start" alignItems="flex-start" spacing={1} style={{ marginBottom: 12 }}>
          <TextTopInGrid sizeXs={12} sizeSm={4} label="Asset No." value={`${regData.Id}-${regData.No}`} />
