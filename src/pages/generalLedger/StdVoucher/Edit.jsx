...
const Edit = (props) => {
  ...
  const { basePath, id, formFields, formFieldsRecurring, formFieldsDetail, translate } = props;
  ...
  const Save = async (values) => {
    ...
    const { Code, InternalMessage } = await updateJvFrDetail(values);
    if (Code === 0) {
    ...
    >
      ...
      <form onSubmit={handleSubmit(onSubmit)} onKeyDown={disableFormEnter}>
        <Paper style={{ padding: 16 }}>
          <BoxHeader header={"Standard JV"} status={data.Status} />
