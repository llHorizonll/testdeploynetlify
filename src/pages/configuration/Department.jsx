...
import DialogItem from "./Dialog/Department";
import CustomTablePagination from "components/CustomTablePagination";
import VisibilityIcon from "@material-ui/icons/Visibility";
import { getDepartmentSearchList } from "services/setting";
...
const Department = (props) => {
  ...
  const fetchSearchList = async (uriQueryString) => {
    const { Data, Paging } = await getDepartmentSearchList(uriQueryString);
    if (Data) {
      setData(Data);
      setPaging(Paging);
    ...
    searchProps: {
      onKeyUp: (e) => {
        if (e.keyCode === 13) {
          ...
          uriQueryString.WhereLikeFields = ["DeptCode", "DeptDesc"];
          fetchSearchList(uriQueryString);
        }
      },
    },
