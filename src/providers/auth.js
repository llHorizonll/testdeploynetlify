...
const authProvider = {
  login: async ({ Username, Password, Tenant, Language }) => {
    ...
    try {
      ...
      localStorage.setItem("Language", DecodeToken.Language);
      localStorage.setItem("adminToken", DecodeToken.AdminToken);
  ...
  getIdentity: async () => {
    try {
      return Promise.resolve({
        id: "user",
        fullName: `${localStorage.getItem("UserName")} @${localStorage.getItem("Tenant")}`,
        //   avatar:
    ...
    try {
      ...
      if (url) {
        ...
        if (callFromDesktopApp) {
          console.log(error, "DashboardDesktopApp in getSettingAll");
          return false;
        } else {
          console.log(error, history, "in auth");
          window.location.replace(url + "#/login");
        }
      }
    }
